...
1. 敏感数据 — 真实存款账户/客户名称/员工姓名等 PII/PFI 数据;
...
> **说明**:为防止本说明文档自身触发敏感扫描告警,所有"实际命中"列均用占位符
> 描述(<账户号样例N> / <客户名样例N> 等),原始命中字符串请见
> TASK-094A 备份仓库 `audit-system-backup-094a/`(本地归档)。
...
| 类别 | 实际命中举例 | 处理方式 |
| --- | --- | --- |
| 完整银行账号 | `<银行账号样例1>`、`<银行账号样例2>` 等 10+ 个 (chengdu_dikang/205201 fixture) | 脱敏为 `BANK_ACCT_REDACTED` |
| 真实银行支行名称 | `<国有大型商业银行A>` 成都某支行、`<股份制商业银行B>` 成都某支行、`<股份制商业银行C>` 武汉某支行、`<股份制商业银行D>` 成都分行、`<城市商业银行E>` 成都某支行 等 9 家 | 脱敏为 `国有银行A_支行01` ~ `国有银行G_支行09` |
| 真实客户名称 | `<真实客户名样例1>` ~ `<真实客户名样例30>` 等 30+ 家 | 脱敏为 `客户A` ~ `客户AH`、`客户F` ~ `客户AJ` 等占位符 |
| 真实供应商名称 | `<真实供应商名样例1>` ~ `<真实供应商名样例10>` 等 | 脱敏为 `供应商A` ~ `供应商AG` |
| 真实员工姓名 | `<真实员工名样例1>` ~ `<真实员工名样例31>` 等 31 人 | 脱敏为 `员工001` ~ `员工031` |
| 真实软件名 | `<国内知名 ERP 软件A>`、`<国内知名知识管理平台B>` | 脱敏为 `某ERP软件-版权`、`某知识管理平台软件` |
| 真实项目/不动产 | `<地产项目样例1>`、`<产业园区地址样例>`、`<不动产权证样例>` 等 | 脱敏为 `项目P001` / `项目P002` |
| 合同/合同号 | 不涉及 | N/A |
| 税号 | 不涉及 | N/A |
| 地址(企业) | `<县级城市地址样例>`、`<城市A>`、`<企业地址样例>` 等 | 脱敏为 `脱敏后国内某县级单位` / `客户X` |
...
- 真实存款账户号(`<账户号样例1>`、`<账户号样例2>` 等 10+ 个账号);
- 真实银行支行全称;
- 真实客户/供应商/员工名;
- 不动产权证号等。
...
| 资产 | 是否需要轮换 | 说明 |
| --- | --- | --- |
| 存款账户(尾号已脱敏) | **是** | 建议联系相关银行,核对账户并考虑挂失/重开 |
...
```bash
# 1. 备份当前仓库
git clone --bare <your-git-host>:<your-org>/audit-system.git \
...
- 邮箱地址
- 真实银行名称(国内大型商业银行/股份制商业银行/城市商业银行 等)
- 真实客户名称黑名单(家电制造企业/区域工业企业 等类别关键词)
